...
% 
prediction_signals = {'HSmag','aAccX','aAccY','aAccZ','d1aOmegaX','d1aOmegaY','d1aOmegaZ','d1aAccX','d1aAccY','d1aAccZ'};
foot_names= {'F1','F2'};
...
for tf = 1:2
    ...
    for ft = 1:2 % foot 1 or 2
        ...
        for cv = 1:FOLDS
            ...
            prevTrainCycles = [mean(trainCycles); trainCycles];
...
%             title('Static prediction training and labels');
            ...
            b = regress(trainCycles, [ftmat_train prevTrainCycles(1:end-1)]);
...
%                     guess = predict(Mdl, ftmat_test);
                    ftmat_test = [ftmat_test prevTestCycles(labiter)];
                    guess = ftmat_test*b;
                    data_pred = [pt_in];
...
%                     end
                    ...
                    LRlabels_ts = [LRlabels_ts (linspace(1,testCycles(labiter)))/testCycles(labiter)];
                    pred_vec = [pred_vec (linspace(1,guess))/guess];
                    labpt = [labpt; repmat(tslab(labiter), [100,1])];
                    labiter = labiter+1;
                end
            end
