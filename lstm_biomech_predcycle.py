...
def readLangs(filter):
    ...
    for f in glob.glob("*.csv"):
        ...
        data_fin[:,0] = np.arange(0,data_fin.shape[0]*avg_int,avg_int)
...
class DataProcessor():
    ...
    def process_data(self):
        self.avg_int = np.mean(self.data[1:,0] - self.data[:-1,0])
...
def train(input_tensor, target_tensor, encoder, decoder, encoder_optimizer,
            decoder_optimizer, criterion, max_length, teacher_forcing_ratio=.5):
