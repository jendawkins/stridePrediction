...
%         prev_cycle_time = prev_cycle_time./max(prev_cycle_time);
        ...
        feat_means=[];
        labels_tr_means=[];
        for fm = 1:size(feature_mat,1)-10
            feat_means(fm,:) = [mean(feature_mat(fm:fm+10,:)) max(feature_mat(fm:fm+10,:))...
                range(feature_mat(fm:fm+10,:))];
            labels_tr_means(fm) = median(LRlabels_tr(fm:fm+10));
        end
%         for feat = 1:size(feature_mat,2)
%             figure
%             autocorr(feature_mat(:,feat))
...
%           Mdl = arima(4,1,1);
%           Ts = 1;
% %           data = iddata(feature_mat(:,1),[],Ts); 
%           na = [0 4];nc = [1];
%           model = ar(y1,1,'ls','Ts',Ts,'IntegrateNoise', true);
%           
%           EstMdl = estimate(Mdl,y1);
%           yf = forecast(model,y1(1:100),100);
%           plot(yf)
...
%         legend('x1','x2','x3')
        ...
        b = regress(labels_tr_means',feat_means + k(6:end-5));
...
%             subplot(2,2,iter)
        ...
        for pt_idx = 1:size(data_test,1)
            ...
            if pt_idx>=11
                pt_means = [mean(data_pred((end-10):end,:)) ...
                    max(data_pred((end-10):end,:)) range(data_pred((end-10):end,:))];
                pred_time = (pt_means + prev_ct)*b;
                
                labels_ts_means(pt_idx-10) = median(LRlabels_ts(pt_idx-10:pt_idx));
                mse_loss = mse_loss + (labels_ts_means(pt_idx-10) - pred_time).^2;
                pred_vec(pt_idx-10) = pred_time;
                labpt(pt_idx-10) = select_labels(test_strides(pl));
            end
                   
%             pred_time = (data_pred + prev_ct)*b;
%             mse_loss = mse_loss + (labels_ts_means(pt_idx-10) - pred_time).^2;
...
%         legend(p)
        ...
        if cv==1
            ...
            gscatter(labels_ts_means, pred_vec, labpt_names');
