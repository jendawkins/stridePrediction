...
NUM_TIMEPOINTS_VEC = 1:2:11; % set to 1 to not use averaging; make vector to check multiple timepoints
ARIMA = 0;
            ...
            for cv = 1:FOLDS
                ...
                if NUM_TIMEPOINTS>1
                    ...
                    for fm = 1:size(feature_mat,1)-NUM_TIMEPOINTS-1
                        ...
                        k(fm) = median(k(fm:fm+NUM_TIMEPOINTS-1));
                    end
                ...
                for pt_idx = 1:size(data_test,1)
                    ...
                    if LRlabels_ts(pt_idx)>=PREDICTION_WINDOW_START
                        ...
                        if size(data_pred,1)>=NUM_TIMEPOINTS
                            if NUM_TIMEPOINTS>1
                                pt_pred = [mean(data_pred) max(data_pred) range(data_pred)];
...
function [dsig] = derivative(signal)
